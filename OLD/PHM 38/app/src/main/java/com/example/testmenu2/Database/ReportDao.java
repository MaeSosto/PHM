...
public interface ReportDao {
    ...
    @Query("SELECT AVG(report_pressione_sistolica) FROM reports WHERE report_giorno=:date AND report_pressione_sistolica != 0")
    LiveData<Double> getAVGPressioneSistolicaInDate(Date date);

    @Query("SELECT AVG(report_pressione_diastolica) FROM reports WHERE report_giorno=:date AND report_pressione_diastolica != 0")
    LiveData<Double> getAVGPressioneDiastolciaInDate(Date date);
    ...
    @Query("SELECT AVG(report_pressione_sistolica) AS media, report_giorno AS giorno FROM reports WHERE report_pressione_sistolica != 0 GROUP BY report_giorno ORDER BY report_giorno")
    LiveData<List<AVG>> getAVGPressioneSistolicaAll();

    @Query("SELECT AVG(report_pressione_diastolica) AS media, report_giorno AS giorno FROM reports WHERE report_pressione_diastolica != 0 GROUP BY report_giorno ORDER BY report_giorno")
    LiveData<List<AVG>> getAVGPressioneDiastolicaAll();
    ...
    @Query("SELECT AVG(report_pressione_sistolica) AS media, report_giorno AS giorno FROM reports WHERE report_pressione_sistolica != 0 AND report_giorno>= :date1 AND report_giorno<=:date2 GROUP BY report_giorno ORDER BY report_giorno")
    LiveData<List<AVG>> getAVGPressioneSistolicaInPeriod(Date date1, Date date2);

    @Query("SELECT AVG(report_pressione_diastolica) AS media, report_giorno AS giorno FROM reports WHERE report_pressione_diastolica != 0 AND report_giorno>= :date1 AND report_giorno<=:date2 GROUP BY report_giorno ORDER BY report_giorno")
    LiveData<List<AVG>> getAVGPressioneDiastolicaInPeriod(Date date1, Date date2);
