...
public class ReportViewModel extends AndroidViewModel {
    ...
    public LiveData<List<Report>> getAllReports(String val, Date date1, Date date2){
        ...
        if(val != null) {
            ...
            if(date2 == null) return reportDao.getAllReports(new SimpleSQLiteQuery(query+ " AND report_giorno= "+ Converters.DateToLong(date1)+ query_final ));
            else return reportDao.getAllReports(new SimpleSQLiteQuery(query+ " AND report_giorno>= "+ Converters.DateToLong(date1)+ " AND report_giorno<="+ Converters.DateToLong(date2)+ query_final ));
        }
        else {
            if (date1 == null && date2 == null) return reportDao.getAllReports(new SimpleSQLiteQuery(query + query_final));
