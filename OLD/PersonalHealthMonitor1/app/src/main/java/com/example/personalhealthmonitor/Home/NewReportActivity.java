        ...
        BTNSendReport.setOnClickListener(v -> {
            if(check2input()){
                if(newReport){
                    ...
                    giorno = calendar.getTime().getDate();
                    mese = (calendar.getTime().getMonth())+1;
                    anno = calendar.getTime().getYear()-100;
                    ora = DateFormat.getTimeInstance(DateFormat.SHORT, Locale.ITALIAN).format(new Date());
                    Report report = new Report(0, giorno, mese, anno, ora, battiti,temperatura,pressioneSistolica, pressioneDiastolica, glicemiamax, glicemiamin, nota);
                    reportViewModel.setReport(report);
                    Toast.makeText(getApplication(), "Report salvato", Toast.LENGTH_SHORT).show();
                    finish();
                }
                else {
                    Report = new Report(reportId, giorno, mese, anno, ora, battiti,temperatura,pressioneSistolica, pressioneDiastolica,glicemiamax, glicemiamin, nota);
