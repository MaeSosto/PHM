...
public class ReportViewModel extends AndroidViewModel {
    ...
    public  int getCount (){
        ...
        return reportDao.getCountReports(new SimpleSQLiteQuery(s));
    }
    /*public LiveData<List<Report>> getCountReports (String value, Date date1, Date date2){
        String string = "SELECT * FROM reports";
        if(value == null) return reportDao.getCountReports(new SimpleSQLiteQuery(string));
        else if(date1 == null && date2== null) return reportDao.getCountReports(new SimpleSQLiteQuery(string));
        else return reportDao.getCountReports(new SimpleSQLiteQuery(string+ "WHERE "+ value+" != 0 AND report_giorno>= "+ Converters.DateToLong(date1)+ " AND report_giorno<= "+ Converters.DateToLong(date2)));
    }
