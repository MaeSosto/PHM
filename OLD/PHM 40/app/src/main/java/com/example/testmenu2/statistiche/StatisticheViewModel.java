...
public class StatisticheViewModel extends ViewModel {
    ...
    public StatisticheViewModel() {
        ...
        periodo.observeForever( new Observer<String>() {
            ...
            public void onChanged(String s) {
                ...
                Log.i("COUNT", String.valueOf(reportViewModel.getCount()));

                /*reportViewModel.getCountReports(null, inizio, fine).observeForever(new Observer<List<Report>>() {
                    @Override
                    public void onChanged(List<Report> reports) {
                        Log.i("COUNT", String.valueOf(reports.size()));
                    }
                });

                 */

                //setTXVnumReport(String.valueOf(reportViewModel.getCountReports(null, inizio, fine)));


                //setTXVnumReport(String.valueOf(reportViewModel.getCountReports(null, null, null)));
    ...
    private PieData getPieData (){
        ...
        //int battito = reportViewModel.getAVG("report_battito", inizio, fine);
        /*int temperatura = reportViewModel.getCountReports("report_temperatura", inizio, fine);
        int pressioneSistolica = reportViewModel.getCountReports("report_pressione_sistolica", inizio, fine);
        int pressioneDiastolica = reportViewModel.getCountReports("report_pressione_diastolica", inizio, fine);
        int glicemia = reportViewModel.getCountReports("report_glicemia", inizio, fine);
        ...
        if(pressioneSistolica != 0) PiedataVals.add(new PieEntry(pressioneSistolica, "Pressione Sistolica"));
        if(pressioneDiastolica != 0) PiedataVals.add(new PieEntry(pressioneDiastolica, "Pressione Diastolica"));
         ...
         */
        //PiedataVals.add(new PieEntry(reportViewModel.getCountReports("report_battito", inizio, fine)), "Battito"));
