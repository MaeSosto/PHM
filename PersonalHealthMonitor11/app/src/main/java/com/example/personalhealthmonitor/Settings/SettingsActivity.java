...
import com.example.personalhealthmonitor.Notification.NotificationBroadcast;
...
import static com.example.personalhealthmonitor.Notification.NotificationBroadcast.NOTIFICATION_CHANNEL_ID;
...
public class SettingsActivity extends AppCompatActivity implements  DatePickerDialog.OnDateSetListener{
    ...
    private Settings battito, pressioneSistolica, pressioneDiastolica, temperatura, glicemiamax, glicemiamin, notification;
    ...
    private boolean saveSettings(Settings settings, SeekBar seekBar, EditText editText, Button button1, Button button2){
        ...
        if(settings.getImportanza() > 2) {
            if(editText.getText().toString().equals("")) return false;
                else settings.setLimite(Double.parseDouble(editText.getText().toString()));
            try {
                settings.setInizio(SDF.parse(button1.getText().toString()));
            } catch (ParseException e) {
                e.printStackTrace();
            }
            try {
                settings.setFine(SDF.parse(button2.getText().toString()));
            } catch (ParseException e) {
                e.printStackTrace();
            }
    ...
    private void saveSettings(){
        ...
        if(notification_switch.isChecked()){
            notification.setImportanza(1);
            Calendar calendar = Calendar.getInstance();
            calendar.set(calendar.getTime().getYear(), calendar.getTime().getMonth(), calendar.getTime().getDate(), timeHour, timeMinute);
            notification.setInizio(calendar.getTime());
        }
        else notification.setImportanza(0);
        ...
        if(ok){
            ...
            settingsViewModel.updateSettings(notification);

            //SE HO SETTATO LE NOTIFICHE ON ALLORA IMPOSTO L'ALARM MANAGER
            if(notification.getImportanza() == 1){
                createNotificationChannel();
                NotificationBroadcast.setNotification(getApplicationContext(), notification.getInizio());
        ...
        for (int i = 0; i<settings.size(); i++){
            ...
            switch (value){
                ...
                case KEY_NOTIFICATION:
                    notification = tmpSetting;
                    if(notification.getImportanza() == 1){
                        notification_switch.setChecked(true);
                        LLnotification.setVisibility(View.VISIBLE);
                        BTNnotification.setText(notification.getInizio().getHours() + ":" + notification.getInizio().getMinutes());
                    }
